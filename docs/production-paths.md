...
- 插件安装后不应依赖构建机路径（例如 `H:\ProgramDocument\...`）。
...
- Windows：`%APPDATA%\zotero-local-mcp-bridge`
- macOS：`~/Library/Application Support/zotero-local-mcp-bridge`
- Linux（含其他类 Unix）：`${XDG_STATE_HOME || XDG_DATA_HOME || ~/.local/share}/zotero-local-mcp-bridge`

可选兼容适配器解析优先级：

1. `ZOTERO_LOCAL_MCP_BRIDGE_RUNTIME_DIR`（显式配置，最高优先）
2. 平台默认运行时目录（上文）

Bootstrap（插件）解析顺序：

1. 插件打包配置显式 `runtimeRoot`（开发/测试可注入）
2. `ZOTERO_LOCAL_MCP_BRIDGE_RUNTIME_DIR`（用户环境变量）
3. Zotero prefs `extensions.zotero-local-mcp-bridge.runtimeRoot`（高级用户可设）
...
- 打包产物内不应出现 `__ZOTERO_LOCAL_MCP_BRIDGE_PROJECT_ROOT__`/`__ZOTERO_LOCAL_MCP_BRIDGE_AUTH_TOKEN__`。
