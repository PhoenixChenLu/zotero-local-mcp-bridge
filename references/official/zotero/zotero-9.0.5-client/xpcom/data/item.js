...
Zotero.Item.prototype.updateDisplayTitle = function () {
  ...
  if (title === "" && (itemTypeID == itemTypeLetter || itemTypeID == itemTypeInterview)) {
    ...
    for (let i = 0; i < creatorsData.length; i++) {
      ...
      if (itemTypeID == itemTypeLetter && creatorTypeID == creatorTypeAuthor ||
  ...
  // 'case'
    ...
    {// civil law cases have only shortTitle as case name
      var strParts = [];
      var caseinfo = "";
  ...
  if (itemTypeID == itemTypeAnnotation) {
    ...
    set: function (value) {
      ...
      switch (name) {
        ...
        case 'sortIndex':{
            ...
            if (parentItem?.isEPUBAttachment()) {
...
Zotero.Item.prototype.multiDiff = function (otherItems, ignoreFields) {
  ...
  for (let i = 0; i < otherItems.length; i++) {
    ...
    for (let i = 0; i < changeset.length; i++) {
      ...
      if (alternatives[change.field].indexOf(change.value) == -1) {
        hasDiffs = true;
